...
// --- Constants ---
const COLORS = ['#25D366', '#34B7F1', '#FBC02D', '#EF4444'];
...
function WhatsappCampaigns() {
  ...
  const [stats, setStats] = useState<CampaignStats>({ totalSent: 0, totalDelivered: 0 }); // New Stats State
  ...
  const renderPagination = () => {
    ...
    // simplified for brevity in this snippet, full logic is in your original code
    return (
      <div className="flex gap-1">
        ...
        <span className="px-3 py-1 border bg-gray-100">{currentPage} of {totalPages}</span>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8">
          ...
          <div className="relative w-full md:w-96">
            <input
              ...
              className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl shadow-sm focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition"
            />
            <FaSearch className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" />
          </div>
        </div>

        {/* --- Stats Summary Cards (NEW) --- */}
        {!loading && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-green-500 flex items-center justify-between transition hover:shadow-lg">
              <div>
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                      {paginatedData.length > 0 ? (
                        paginatedData.map((row, index) => {
                          ...
                          const statusColor = row.status === 'COMPLETED' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-yellow-100 text-yellow-700 border-yellow-200';
                          return (
...
>
  Download Report
      ...
      {/* --- Analytics Modal (Existing Logic) --- */}
      {showAnalytics && selectedCampaign && (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-75 z-50 flex justify-center items-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            {/* ... Modal Content remains similar to previous version, just styled ... */}
                ...
                <div className="w-full lg:w-2/5 space-y-4">
                  <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                    ...
                    <div className="space-y-3 text-sm">
                      ...
                      <DetailRow label="Status" value={<span className="text-green-600 font-bold">{selectedCampaign.status}</span>} />
