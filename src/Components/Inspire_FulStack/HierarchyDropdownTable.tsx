...
//   const { smdata, cludata, vddata, loading, error } = useHierarchyData();
...
//   // --- Effects ---
//   useEffect(() => {
//     if (!cludata.length && !smdata.length) return;
//     const data = activeSection === "cluster" ? cludata : smdata;
//     setTree(buildTree(data, activeSection, vddata));
//     setExpandedNodes(new Set()); setSelectedItemsMap(new Map()); setIsSubmitted(false);
//   }, [activeSection, smdata, cludata, vddata, buildTree]);
...
function HierarchyDropdownTable() {
  return (
    <div>HierarchyDropdownTable</div>
  )
}
